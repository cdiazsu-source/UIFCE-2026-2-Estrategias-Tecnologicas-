 ...
 *
 *  Cada hallazgo también trae un `topic`: el tema concreto del que habla (ej.
 *  "MicroTalleres", "Extensión Solidaria"), para que la interpretación no
 *  quede flotando sin decir de qué está hablando.
 *
...
/** `topic`: de qué trata el hallazgo, en 1-3 palabras (se muestra antes de la
 *  interpretación). `thesis`: la lectura/interpretación del hallazgo — qué
 *  implica para la estrategia del área, no solo el dato. Es la síntesis, el
 *  dato bruto es `text`. */
export type DofaItem = { topic: string; text: string; source?: string; period: DofaPeriod; thesis: string };
