...
export async function applyUndo(action: UndoAction) {
  ...
  switch (action.kind) {
    ...
    case "project.priority": {
      await prisma.project.update({ where: { id: action.id }, data: { priorityTag: action.before } });
      revalidatePath(`/proyectos/${action.id}`);
      break;
    }
    ...
    case "project.requester": {
      await prisma.project.update({ where: { id: action.id }, data: { requesterId: action.before } });
      revalidatePath(`/proyectos/${action.id}`);
      break;
    }
    case "project.title": {
